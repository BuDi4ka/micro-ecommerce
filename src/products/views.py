...
from micro_ecom.storages.utils import generate_presigned_url
...
def product_attachment_download(request, handle=None, pk=None):
    ...
    if request.user.is_authenticated and can_download is False:
        can_download = request.user.purchase_set.filter(product=attachment.product, completed=True).exists()
